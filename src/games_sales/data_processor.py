...
class DataProcessor:
    def __init__(self, pandas_df: pd.DataFrame, config: ProjectConfig, spark: SparkSession | None = None):
        self.config = config
        self.df = pandas_df
        self.spark = spark
    ...
    def _aggregate_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for aggregation in self.config.preprocessing['aggregations']:
            if aggregation == 'counts':
                columns_mapping = {col: f'{col}_count' for col in self.config.preprocessing['aggregations'][aggregation]}
                df_agg_X = df_grouped[self.config.preprocessing['aggregations'][aggregation]].count().rename(columns=columns_mapping)
    ...
    def _drop_short_series(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        years_counts = df.groupby(self.config.preprocessing['aggregation_level'])[self.config.preprocessing['date_column']].transform('count')
    ...
    def split_by_time(self, test_periods=12, min_train_periods=24):
        """
        Split dataframe into train and test sets, taking k last periods for test
        for each group if enough data points are available.
